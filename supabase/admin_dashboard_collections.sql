...
create table if not exists public.study_plans (
  ...
  postgrad_cs text,
  postgrad_ai text,
  professional_diplomas jsonb,
  is_published boolean not null default false,
  published_at timestamptz,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint study_plans_professional_diplomas_array check (
    professional_diplomas is null or jsonb_typeof(professional_diplomas) = 'array'
  )
);
...
create table if not exists public.schedules (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint schedules_type_valid check (schedule_type in ('Normal', 'Quiz 1', 'Quiz 2', 'Final')),
